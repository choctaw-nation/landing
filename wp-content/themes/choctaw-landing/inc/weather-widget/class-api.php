...
class API {
	 ...
	 * @var $base_url
	 */
	protected string $base_url = 'https://api.openweathermap.org/data/2.5/forecast?lat=34.1418916&lon=-94.7446644&cnt=7&units=imperial';
	...
	protected function get_weather_data(): array|string|null {
		...
		if ( isset( $response['response']['message'] ) && 'OK' !== $response['response']['message'] ) {
			$response = new \WP_Error( 'weather_api', $response['response']['message'], $response['headers']['data'] );
		}
